...
__global__ void cosine_angle_cuda_kernel(
    scalar_t* coords,
    int64_t* angles,
    scalar_t* theta0,
    scalar_t* k,
    ...
    for (int index = threadIdx.x + blockIdx.x * BLOCK_SIZE; index < nangles;
        ...
        scalar_t theta0_i = theta0[index];
        scalar_t c0 = cos_(theta0_i);
        scalar_t dc = cos_theta - c0;

        scalar_t k_ = k[index];

        if (ene_out) {
            ene += k_ * dc * dc / scalar_t(2.0);
        }

        if (coord_grad) {
            scalar_t prefix = k_ * dc / (v1_norm * v2_norm) * sign;
        ...
        if (k_grad) {
            k_grad[index] = dc * dc / scalar_t(2.0);
        }
        if (theta0_grad) {
            theta0_grad[index] = k_ * dc * sin_(theta0_i);
        }
    }
    if (ene_out) {
...
class CosineAngleFunctionCuda : public torch::autograd::Function<CosineAngleFunctionCuda> {
public:
    static at::Tensor forward(
        ...
        AT_DISPATCH_FLOATING_TYPES(coords.scalar_type(), "compute_cosine_angle_cuda", ([&] {
            cosine_angle_cuda_kernel<scalar_t, BLOCK_SIZE><<<GRID_SIZE, BLOCK_SIZE, 0, stream>>>(
...
at::Tensor compute_cosine_angle_energy_cuda(
    at::Tensor& coords,
    at::Tensor& angles,
    at::Tensor& theta0,
    at::Tensor& k
) {
    return CosineAngleFunctionCuda::apply(coords, angles, theta0, k);
}

void compute_cosine_angle_forces_cuda(
    at::Tensor& coords,
    at::Tensor& angles,
    at::Tensor& theta0,
    at::Tensor& k,
    ...
    AT_DISPATCH_FLOATING_TYPES(coords.scalar_type(), "compute_cosine_angle_forces_cuda", ([&] {
        cosine_angle_cuda_kernel<scalar_t, BLOCK_SIZE><<<GRID_SIZE, BLOCK_SIZE, 0, stream>>>(
            coords.data_ptr<scalar_t>(),
            angles.data_ptr<int64_t>(),
            theta0.data_ptr<scalar_t>(),
...
TORCH_LIBRARY_IMPL(torchff, AutogradCUDA, m) {
    m.impl("compute_cosine_angle_energy", compute_cosine_angle_energy_cuda);
    m.impl("compute_cosine_angle_forces", compute_cosine_angle_forces_cuda);
}
