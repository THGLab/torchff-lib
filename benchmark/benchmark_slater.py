"""
Slater pair potential timing: reference (pure PyTorch) vs TorchFF custom ops.

Builds a liquid-like box (water-like density), neighbor list (nsquared, 0.8 nm cutoff),
then benchmarks :class:`torchff.slater.Slater`. Timing includes forward plus backward;
only ``coords`` has ``requires_grad`` (A/B are fixed).
...
Requires CUDA. Run from the repo root with torchff installed::
    ...
    python benchmark/benchmark_slater.py
    python benchmark/benchmark_slater.py --dtype float64 --atoms 1000 10000
"""
...
def create_benchmark_data(num: int, density: float, cutoff, device, dtype):
    ...
    box_len = (num / density) ** (1 / 3)
    coords = np.random.rand(num, 3) * box_len
...
def _compile_slater(module: Slater, compile_mode: str) -> torch.nn.Module:
...
def _slater_tensors_for_size(
    ...
    """Create coords (requires_grad), box, pairs, cutoff, A, B; return None if no pairs."""
    ...
    A = 0.5 + 1.5 * torch.rand(num_pairs, device=device, dtype=dtype, generator=gen)
    B = 0.2 + 0.8 * torch.rand(num_pairs, device=device, dtype=dtype, generator=gen)

    coords = coords.clone().detach().requires_grad_(True)
    A = A.clone().detach()
    B = B.clone().detach()

    return coords, box, pairs, cutoff, A, B, num_pairs


def benchmark_slater_model(
    num_atoms: int,
    density: float,
    device: str,
    dtype: torch.dtype,
    ...
    """Run ref + torch.compile(torchff); return two CSV rows or None."""
    packed = _slater_tensors_for_size(num_atoms, density, device, dtype)
    if packed is None:
        print(
    ...
    coords, box, pairs, cutoff, A, B, num_pairs = packed
    dtype_s = _dtype_str(dtype)

    func = _compile_slater(
        Slater(
            use_customized_ops=True,
    ...
    perf_ref = perf_op(
        ...
        B,
        cutoff,
        desc=f"ref-slater N={num_atoms} P={num_pairs} {dtype_s}",
        warmup=warmup,
        repeat=repeat,
        run_backward=True,
        use_cuda_graph=True,
    ...
    perf_tf = perf_op(
        ...
        B,
        cutoff,
        desc=f"torchff-slater N={num_atoms} P={num_pairs} {dtype_s}",
        warmup=warmup,
        repeat=repeat,
        run_backward=True,
        use_cuda_graph=True,
...
def write_slater_csv(rows: List[Dict[str, Any]], csv_path: Path) -> None:
    fieldnames = [
        "dtype",
        "num_atoms",
        "num_pairs",
...
def plot_slater_benchmark(rows: List[Dict[str, Any]], pdf_path: Path) -> None:
    """Log-log: Slater ref vs torchff."""
    ...
    ax.set_title("Slater pair potential")
    ax.set_xlabel("Number of pairs")
    ax.set_ylabel("Time (ms)")
    ax.grid(True, which="both", alpha=0.35)
    sub = [r for r in rows if r["model"] == "slater"]
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "torch.compile Inductor mode. Inductor triton.cudagraphs is forced off so it does not "
            "nest with perf_op's outer CUDA graph."
        ),
    )
    parser.add_argument(
        "-o",
    ...
    if all_rows:
        write_slater_csv(all_rows, out_csv)
        print(f"Summary: {len(all_rows)} rows")

    if not args.no_plot and all_rows:
        plot_slater_benchmark(all_rows, out_pdf)
