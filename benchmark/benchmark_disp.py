"""
Tang–Tonnies C6 dispersion timing: reference (pure PyTorch) vs TorchFF custom ops.

Same layout as ``benchmark_vdw.py``: liquid-like density, ``NeighborList`` (nsquared),
forward+backward with only ``coords`` differentiable.

Requires CUDA. Run from the repo root with torchff installed::

    python benchmark/benchmark_disp.py
"""
...
def _disp_tensors_for_size(
    ...
    c6 = 0.5 + 1.5 * torch.rand(num_pairs, device=device, dtype=dtype, generator=gen)
    b = 1.0 + 2.0 * torch.rand(num_pairs, device=device, dtype=dtype, generator=gen)

    coords = coords.clone().detach().requires_grad_(True)
    c6 = c6.clone().detach()
    b = b.clone().detach()

    return coords, box, pairs, cutoff, c6, b, num_pairs


def benchmark_dispersion_model(
    num_atoms: int,
    density: float,
    device: str,
    dtype: torch.dtype,
    ...
    packed = _disp_tensors_for_size(num_atoms, density, device, dtype)
    if packed is None:
        print(
    ...
    coords, box, pairs, cutoff, c6, b, num_pairs = packed
    dtype_s = _dtype_str(dtype)

    func = _compile_dispersion(
        Dispersion(
            use_customized_ops=True,
    ...
    perf_ref = perf_op(
        ...
        b,
        cutoff,
        desc=f"ref-tang_tonnies N={num_atoms} P={num_pairs} {dtype_s}",
        warmup=warmup,
        repeat=repeat,
        run_backward=True,
        use_cuda_graph=True,
    ...
    perf_tf = perf_op(
        ...
        b,
        cutoff,
        desc=f"torchff-tang_tonnies N={num_atoms} P={num_pairs} {dtype_s}",
        warmup=warmup,
        repeat=repeat,
        run_backward=True,
        use_cuda_graph=True,
...
def write_disp_csv(rows: List[Dict[str, Any]], csv_path: Path) -> None:
    fieldnames = [
        "dtype",
        "num_atoms",
        "num_pairs",
...
def plot_disp_benchmark(rows: List[Dict[str, Any]], pdf_path: Path) -> None:
    ...
    ax.set_title("Tang–Tonnies C6 dispersion")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=100.0,
        help="Atom number density (atoms / nm^3).",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        help="torch.compile Inductor mode (triton.cudagraphs forced off).",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    for dt in dtypes:
        ...
        for n in args.atoms:
            print(f"\n--- N={n} ---")
            rows = benchmark_dispersion_model(
                num_atoms=n,
    ...
    if all_rows:
        write_disp_csv(all_rows, out_csv)
        print(f"Summary: {len(all_rows)} rows")

    if not args.no_plot and all_rows:
        plot_disp_benchmark(all_rows, out_pdf)
